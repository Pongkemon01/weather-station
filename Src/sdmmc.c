...
void HAL_SD_MspInit(SD_HandleTypeDef *sdHandle)
{
  ...
  if (sdHandle->Instance == SDMMC1)
  {
    ...
    */
    ...
    //  Replace both hdma_sdmmc1_rx and hdma_sdmmc1_tx with a single handle
    ...
    // Note: Direction gets set internally by HAL_SD before each transfer,
    // so a single static Direction field here isn't used the same way —
    // HAL_SD_Read/WriteBlocks_DMA reconfigure direction via SDMMC_DMA APIs.
