/**
 * @file    crc32.c
 * @brief   Fast table-based CRC-32/MPEG-2 implementation.
 *
 * See crc32.h for algorithm parameters and usage notes.
 *
 * Table storage
 * -------------
 * The 256-entry lookup table is a pre-computed const array placed in Flash
 * (.rodata) by the linker.  Cost: 1 KB Flash, 0 bytes SRAM, 0 init cycles.
 *
 * Update formula (non-reflected / MSB-first CRC):
...
/* ── Pre-computed CRC-32/MPEG-2 lookup table (poly 0x04C11DB7, MSB-first) ── */
...
uint32_t crc32_update(uint32_t crc, const uint8_t *data, uint32_t len)
{
    ...
    for (uint32_t i = 0u; i < len; i++)
        crc = (crc << 8u) ^ s_table[((crc >> 24u) ^ (uint32_t)data[i]) & 0xFFu];
