...
AtReadyResult_t at_channel_check_network_ready(uint32_t total_timeout_ms)
{
    ...
    for (int c = 0; c < 2; c++)
    {
        ...
        AtResult_t r = at_channel_send_cmd(cmds[c], total_timeout_ms);
                                      ...
                                      uint8_t at_alive_retries)
{
    ...
    /* ── Step 3: LTE/packet domain attach ──────────────────────────── */
    {
        printf("Waiting for LTE data service...\r\n");
        char cap[32];
        bool attached = false;
...
AtResult_t at_channel_send_cntp(uint32_t timeout_ms)
{
    ...
    bool tx_ok =
        UART_Sys_Send(s_ch.uart_ctx, (const uint8_t *)"AT+CNTP\r\n", 9u, AT_COMM_TIMEOUT);
...
static bool is_http_urc(const char *line)
{
    return strncmp(line, "+HTTPACTION:", 12) == 0 || strncmp(line, "+HTTP_PEER_CLOSED", 17) == 0 || strncmp(line, "+HTTP_NONET_EVENT", 17) == 0;
}
